...
async function openDatabase(): Promise<Database> {
  ...
  const d = await Database.load('sqlite:acg.db');
  await migrate(d);
  return d;
}
...
export async function insertWork(input: WorkInput): Promise<number> {
  ...
  const res = await d.execute(
    `INSERT INTO works (title, category, year, season, status, total_count, current_count, rating, my_rating, synopsis, tags, notes, cover_path, links, source, created_at, updated_at)
    ...
    [
      ...
      input.synopsis, input.tags, input.notes, input.cover_path, input.links,
...
export async function updateWork(id: number, input: WorkInput): Promise<void> {
  ...
  const now = new Date().toISOString();
  await d.execute(
    `UPDATE works SET title=$1, category=$2, year=$3, season=$4, status=$5, total_count=$6, current_count=$7, rating=$8, my_rating=$9, synopsis=$10, tags=$11, notes=$12, cover_path=$13, links=$14, source=$15, updated_at=$16 WHERE id=$17`,
    [
      input.title, input.category, input.year, input.season, input.status,
      input.total_count, input.current_count, input.rating, input.my_rating,
      input.synopsis, input.tags, input.notes, input.cover_path, input.links,
